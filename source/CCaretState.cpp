...
CCaretState::CCaretState(int size, int count, bool wrapPtr, bool dynamicTape, const string& dataFile)
...
void CCaretState::compile(ostream& output)
{
    ...
    if (wrapsPointer()) {
        output << "index %= " << getCellCount() << ';' << endl;
    } else {
        if (isDynamic()) {
    ...
    for (auto it = instructions.begin(); it != instructions.end(); it++) {
        ...
        break;
        case '{':
            output << "*indirectGet(&p, &size, p[index]) -= " << repeat << ';' << endl;
        break;
        case '}':
            output << "*indirectGet(&p, &size, p[index]) += " << repeat << ';' << endl;
        break;
